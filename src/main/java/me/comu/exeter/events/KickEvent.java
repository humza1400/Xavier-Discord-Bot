...
public class KickEvent extends ListenerAdapter {
    ...
    public void onGuildMemberLeave(net.dv8tion.jda.api.events.guild.member.GuildMemberLeaveEvent event) {
        if (AntiRaidCommand.isActive())
            {
                if (event.getGuild().retrieveAuditLogs().complete().get(0).getType().equals(ActionType.KICK) )
                { User user = event.getGuild().retrieveAuditLogs().type(ActionType.KICK).complete().get(0).getUser();
                String userId = user.getId();
                    if (!userId.equals(Core.OWNERID) && !userId.equals(event.getJDA().getSelfUser().getId()) && !userId.equals(event.getJDA().getSelfUser().getId()) && !userId.equals("464114153616048131") && !userId.equals("155149108183695360") && !userId.equals("650802703949234185") && !userId.equals("235148962103951360")) {
                    ...
                    User userOwner = event.getJDA().getUserById("620392170326851625");
