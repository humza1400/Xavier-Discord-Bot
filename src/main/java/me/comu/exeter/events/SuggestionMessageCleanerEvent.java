...
public class SuggestionMessageCleanerEvent extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(@Nonnull GuildMessageReceivedEvent event) {
        ...
        {
            if (!Objects.requireNonNull(event.getMember()).hasPermission(Permission.ADMINISTRATOR) && !event.getMessage().getAuthor().getId().equalsIgnoreCase(event.getJDA().getSelfUser().getId())) {
