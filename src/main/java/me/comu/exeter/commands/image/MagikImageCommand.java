...
public class MagikImageCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        if (args.isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            event.getChannel().sendMessage("Please specify an image to magik").queue();
            return;
        }

        String url = "https://nekobot.xyz/api/imagegen?type=magik&intensity=3&image=";
    ...
    public String getHelp() {
        return "Magik's the specified image\n`" + Core.PREFIX + getInvoke() + " [image]`\nAliases: `" + Arrays.deepToString(getAlias()) + "`";
    }
