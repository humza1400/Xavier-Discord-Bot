...
public class BlurpifyImageCommand implements ICommand {


    @Override
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        if (args.isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            event.getChannel().sendMessage("Please specify an image to blurpify").queue();
            return;
        }

        String url = "https://nekobot.xyz/api/imagegen?type=blurpify&image=";
    ...
    public String getHelp() {
        return "Blurpify the specified image\n`" + Core.PREFIX + getInvoke() + " [image]`\nAliases: `" + Arrays.deepToString(getAlias()) + "`";
    }
    ...
    public String[] getAlias() {
        return new String[] {"blurp","blurpimage","blurpimg","blurpifyimage","blurpifyimg"};
    }
