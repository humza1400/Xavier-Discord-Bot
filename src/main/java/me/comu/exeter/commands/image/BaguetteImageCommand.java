...
public class BaguetteImageCommand implements ICommand {


    @Override
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        if (args.isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            event.getChannel().sendMessage("Please specify an image to baguette-ify").queue();
            return;
        }
        String url = "https://nekobot.xyz/api/imagegen?type=baguette&url=";
    ...
    public String getHelp() {
        return "Baguette-ify the specified image\n`" + Core.PREFIX + getInvoke() + " [image]`\nAliases: `" + Arrays.deepToString(getAlias()) + "`";
    }
