...
public class GlowImageCommand implements ICommand {
        ...
        if (event.getMessage().getAttachments().isEmpty()) {
            ...
            event.getChannel().sendMessage("`Processing Image...`").queue(message -> {
                try {
                    ...
                    ImageIO.write(glowImg(image), "png", newFilePNG);
    ...
    private static BufferedImage glowImg(BufferedImage image) {
        new JFXPanel();
    ...
    public String getHelp() {
        return "Adds a glow filter to the specified image\n`" + Core.PREFIX + getInvoke() + " [image]`\nAliases: `" + Arrays.deepToString(getAlias()) + "`";
    }
    ...
    public String[] getAlias() {
        return new String[]{"glowfilter", "glowimage", "glowimg"};
    }
