...
public class DeepFryImageCommand implements ICommand {


    @Override
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        if (args.isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            event.getChannel().sendMessage("Please specify an image to deep fry").queue();
            return;
        }

        String url = "https://nekobot.xyz/api/imagegen?type=deepfry&image=";
    ...
    public String getHelp() {
        return "Deep frys the specified image\n`" + Core.PREFIX + getInvoke() + " [image]`\nAliases: `" + Arrays.deepToString(getAlias()) + "`";
    }
    ...
    public String[] getAlias() {
        return new String[] {"fry","deepfryimage","deepfryimg","fryimage","fryimg"};
    }
