    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        try {
            ...
            for (int i = 0; i < input; i++) {
                event.getGuild().createTextChannel("champagnepapi" + (this.getRandom())).queue();
                if (i == input - 1) {
...
//                    event.getChannel().sendMessage(eb.build()).queue();
            ...
            event.getChannel().sendMessageEmbeds(Utility.errorEmbed("Insert an amount of text channels to create, dummy!").build()).queue();
        }
