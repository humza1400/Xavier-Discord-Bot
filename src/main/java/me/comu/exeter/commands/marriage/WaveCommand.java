...
public class WaveCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        if (args.isEmpty()) {
            event.getChannel().sendMessageEmbeds(Utility.embedImage(waveUrls[new Random().nextInt(waveUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** waves at themselves :flushed:", event.getMember().getEffectiveName())).build()).queue();
            return;
        }
        ...
        if (!args.isEmpty() && mentionedMembers.isEmpty()) {
            ...
            event.getChannel().sendMessageEmbeds(Utility.embedImage(waveUrls[new Random().nextInt(waveUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** waves at **%s**", event.getMember().getEffectiveName(), targets.get(0).getEffectiveName())).build()).queue();
        } else if (!args.isEmpty()) {
            event.getChannel().sendMessageEmbeds(Utility.embedImage(waveUrls[new Random().nextInt(waveUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** waves at **%s**", event.getMember().getEffectiveName(), mentionedMembers.get(0).getEffectiveName())).build()).queue();
        }
    }
