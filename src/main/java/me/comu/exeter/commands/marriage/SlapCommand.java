...
public class SlapCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        if (args.isEmpty())
        {
            event.getChannel().sendMessage(EmbedUtils.embedImage(slapUrls[new Random().nextInt(slapUrls.length)]).setColor(event.getMember().getColor()).setTitle(String.format("**%s** slaps themselves :flushed:", event.getMember().getEffectiveName())).build()).queue();
            return;
        }
        ...
        if (!args.isEmpty() && mentionedMembers.isEmpty()) {
            ...
            event.getChannel().sendMessage(EmbedUtils.embedImage(slapUrls[new Random().nextInt(slapUrls.length)]).setColor(event.getMember().getColor()).setTitle(String.format("**%s** slaps **%s**", event.getMember().getEffectiveName(), targets.get(0).getEffectiveName())).build()).queue();
            return;
        }
        else if (!args.isEmpty() && !mentionedMembers.isEmpty())
        {
            event.getChannel().sendMessage(EmbedUtils.embedImage(slapUrls[new Random().nextInt(slapUrls.length)]).setColor(event.getMember().getColor()).setTitle(String.format("**%s** slaps **%s**", event.getMember().getEffectiveName(), mentionedMembers.get(0).getEffectiveName())).build()).queue();
        }
    }
