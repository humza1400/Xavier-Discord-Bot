...
public class BiteCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        if (args.isEmpty()) {
            event.getChannel().sendMessageEmbeds(Utility.embedImage(biteUrls[new Random().nextInt(biteUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** bites themselves :flushed:", event.getMember().getEffectiveName())).build()).queue();
            return;
        }
        ...
        if (!args.isEmpty() && mentionedMembers.isEmpty()) {
            ...
            event.getChannel().sendMessageEmbeds(Utility.embedImage(biteUrls[new Random().nextInt(biteUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** bites **%s**", event.getMember().getEffectiveName(), targets.get(0).getEffectiveName())).build()).queue();
        } else if (!args.isEmpty()) {
            event.getChannel().sendMessageEmbeds(Utility.embedImage(biteUrls[new Random().nextInt(biteUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** bites **%s**", event.getMember().getEffectiveName(), mentionedMembers.get(0).getEffectiveName())).build()).queue();
        }
    }
