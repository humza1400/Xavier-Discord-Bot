...
public class HoldHandsCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        if (args.isEmpty()) {
            event.getChannel().sendMessageEmbeds(Utility.embedImage(holdHandsUrls[new Random().nextInt(holdHandsUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** holds his own hands :flushed:", event.getMember().getEffectiveName())).build()).queue();
            return;
        }
        ...
        if (!args.isEmpty() && mentionedMembers.isEmpty()) {
            ...
            event.getChannel().sendMessageEmbeds(Utility.embedImage(holdHandsUrls[new Random().nextInt(holdHandsUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** holds **%s**'s hands", event.getMember().getEffectiveName(), targets.get(0).getEffectiveName())).build()).queue();
        } else if (!args.isEmpty()) {
            event.getChannel().sendMessageEmbeds(Utility.embedImage(holdHandsUrls[new Random().nextInt(holdHandsUrls.length)]).setColor(Core.getInstance().getColorTheme()).setTitle(String.format("**%s** holds **%s**'s hands", event.getMember().getEffectiveName(), mentionedMembers.get(0).getEffectiveName())).build()).queue();
        }
    }
