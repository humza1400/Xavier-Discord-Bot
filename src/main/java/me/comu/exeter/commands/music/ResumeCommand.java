...
public class ResumeCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        if (!Objects.requireNonNull(voiceChannel).getMembers().contains(event.getMember())) {
            event.getChannel().sendMessageEmbeds(Utility.errorEmbed("You need to be in the same voice channel as me to resume a queue").build()).queue();
            return;
        }
        guildMusicManager.player.setPaused(false);
        event.getChannel().sendMessageEmbeds(Utility.embed("Resumed the queue.").build()).queue();
