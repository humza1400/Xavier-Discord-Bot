...
public class FMLCommand implements ICommand {
    @Override
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        String url = "https://api.alexflipnote.dev/fml";
        ...
        okHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                if (response.isSuccessful()) {
                    ...
                    String text = jsonObject.toMap().get("text").toString();
                    event.getChannel().sendMessageEmbeds(Utility.embed(text).build()).queue();
                } else {
                    event.getChannel().sendMessageEmbeds(Utility.errorEmbed(Utility.ERROR_EMOTE + " Something went wrong making a request to the endpoint").build()).queue();
                }
